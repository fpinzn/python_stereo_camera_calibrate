...
##############################################
# Final CSV header: frame,
# then for front: front_<fixed fields> and for each landmark front_<landmark>_x, front_<landmark>_y, front_<landmark>_c,
# then for side: side_<bounding_box_js_x0...bounding_box_js_y1> and side_<landmark>_x, side_<landmark>_y, side_<landmark>_c.
finalHeader="frame,"
# Front fixed fields.
IFS=',' read -ra ff <<< "$frontFixed"
frontFixedPrefixed=""
for f in "${ff[@]}"; do
    if [ -z "$frontFixedPrefixed" ]; then
        frontFixedPrefixed="front_${f}"
    else
        frontFixedPrefixed+=",front_${f}"
    fi
done
finalHeader+="$frontFixedPrefixed,"
# Front landmarks.
IFS=',' read -ra fl <<< "$frontLandmarks"
frontLMHeaders=""
for l in "${fl[@]}"; do
    if [ -z "$frontLMHeaders" ]; then
        frontLMHeaders="front_${l}_x,front_${l}_y,front_${l}_c"
    else
        frontLMHeaders+=",front_${l}_x,front_${l}_y,front_${l}_c"
    fi
done
finalHeader+="$frontLMHeaders,"
# Side fixed fields: we only want bounding box fields (drop absolute_time and relative_time).
IFS=',' read -ra sf <<< "$sideFixed"
sideFixedArray=($sideFixed)
if [ ${#sideFixedArray[@]} -ge 6 ]; then
    sideFixedPrefixed="side_${sideFixedArray[2]},side_${sideFixedArray[3]},side_${sideFixedArray[4]},side_${sideFixedArray[5]}"
else
    sideFixedPrefixed=""
fi
finalHeader+="$sideFixedPrefixed,"
# Side landmarks.
IFS=',' read -ra sl <<< "$sideLandmarks"
sideLMHeaders=""
for l in "${sl[@]}"; do
    if [ -z "$sideLMHeaders" ]; then
        sideLMHeaders="side_${l}_x,side_${l}_y,side_${l}_c"
    else
        sideLMHeaders+=",side_${l}_x,side_${l}_y,side_${l}_c"
    fi
done
finalHeader+="$sideLMHeaders"
OUTPUT_CSV="video_landmarks.csv"
echo "$finalHeader" > "$OUTPUT_CSV"
echo "CSV header written."

##############################################
# Step 6: Merge NDJSON lines from front and side files into CSV rows.
##############################################
numFrames=$(wc -l < front_video.json)
for (( i=1; i<=numFrames; i++ )); do
    frontLine=$(sed -n "${i}p" front_video.json)
    sideLine=$(sed -n "${i}p" side_video.json)
    frame=$(echo "$frontLine" | jq -r '.frame')
    ftime=$(echo "$frontLine" | jq -r '.absolute_time')
    frel=$(echo "$frontLine" | jq -r '.relative_time')
    fbbox_x0=$(echo "$frontLine" | jq -r '.bounding_box.x0')
    fbbox_y0=$(echo "$frontLine" | jq -r '.bounding_box.y0')
    fbbox_x1=$(echo "$frontLine" | jq -r '.bounding_box.x1')
    fbbox_y1=$(echo "$frontLine" | jq -r '.bounding_box.y1')
    frontLM=""
    for key in $(echo "$frontHeaderJSON" | jq -r '.landmark_keys | join(",")'); do
        lx=$(echo "$frontLine" | jq -r ".landmarks[\"$key\"].x")
        ly=$(echo "$frontLine" | jq -r ".landmarks[\"$key\"].y")
        lc=$(echo "$frontLine" | jq -r ".landmarks[\"$key\"].c")
        frontLM+="$lx,$ly,$lc,"
    done
    frontLM=${frontLM%,}
    
    sbbox_x0=$(echo "$sideLine" | jq -r '.bounding_box.x0')
    sbbox_y0=$(echo "$sideLine" | jq -r '.bounding_box.y0')
    sbbox_x1=$(echo "$sideLine" | jq -r '.bounding_box.x1')
    sbbox_y1=$(echo "$sideLine" | jq -r '.bounding_box.y1')
    sideLM=""
    for key in $(echo "$sideHeaderJSON" | jq -r '.landmark_keys | join(",")'); do
        lx=$(echo "$sideLine" | jq -r ".landmarks[\"$key\"].x")
        ly=$(echo "$sideLine" | jq -r ".landmarks[\"$key\"].y")
        lc=$(echo "$sideLine" | jq -r ".landmarks[\"$key\"].c")
        sideLM+="$lx,$ly,$lc,"
    done
    sideLM=${sideLM%,}
    echo "${frame},${ftime},${frel},${fbbox_x0},${fbbox_y0},${fbbox_x1},${fbbox_y1},${frontLM},${sbbox_x0},${sbbox_y0},${sbbox_x1},${sbbox_y1},${sideLM}" >> "$OUTPUT_CSV"
done
